...
async def backup_info(event, client, backup_folder):
    try:
        ...
        await event.reply("Information backup complete!")

    except Exception as e:
        await event.reply(f"An error occurred during backup: {str(e)}")
...
async def revert_info(event, client, backup_folder):
    try:
        ...
        if not await check_files_in_backup_folder(backup_folder):
            await event.reply("No backup files found.")
            return
        ...
        await event.reply("Restore complete!")

    except Exception as e:
        await event.reply(f"An error occurred while restoring: {str(e)}")
